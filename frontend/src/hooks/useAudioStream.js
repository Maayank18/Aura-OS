...
export default function useAudioStream() {
  ...
  // Removed processTranscript since it is now handled by the background telemetryStore loop
  ...
  const start = useCallback(
    async (visualizerCanvas) => {
      try {
        ...
        if (isDesktop) {
          // Electron Mode: Use MediaRecorder + Groq Backend
          ...
          mediaRecorder.onstart = () => {
            ...
            setAuraTranscript('Listening (Native Mode)...');
          };
          
          mediaRecorder.start();
        } else {
          // Web Mode: Use Web Speech API
  ...
  const stop = useCallback(async () => {
    ...
    useTelemetryStore.getState().stopTelemetryLoop();

    const vols = volumeHistoryRef.current;
    const avgVolume = vols.length > 0 ? vols.reduce((a, b) => a + b, 0) / vols.length : 0;
    
    if (isDesktop && mediaRecorderRef.current && mediaRecorderRef.current.state !== 'inactive') {
      mediaRecorderRef.current.onstop = async () => {
        const audioBlob = new Blob(audioChunksRef.current, { type: 'audio/webm' });
        const durationMins = (Date.now() - transcriptStartTimeRef.current) / 60000;
        
        try {
          setAuraTranscript('Transcribing audio natively...');
          const result = await clinicalApi.transcribeAudio(audioBlob);
          const finalTranscript = result.text;
          
          let wpm = 0;
          if (durationMins > 0 && finalTranscript) {
             const words = finalTranscript.split(/\s+/).filter(w => w.length > 0).length;
             wpm = Math.round(words / durationMins);
          }
          
          latestTranscriptRef.current = finalTranscript;
          setAuraTranscript(finalTranscript);
          
          cleanupAudioSession({ cancelSpeech: false, clearTranscript: false });
          setAuraSpeaking(false);
          await useStore.getState().dispatchVoiceTelemetry(finalTranscript, wpm, avgVolume);
        } catch (e) {
          console.error('Transcription error:', e);
          setAuraTranscript('[Transcription failed: ' + e.message + ']');
          cleanupAudioSession({ cancelSpeech: false, clearTranscript: false });
          setAuraSpeaking(false);
        }
      };
      ...
      let wpm = 0;
      if (transcriptStartTimeRef.current && finalTranscript) {
        const durationMins = (Date.now() - transcriptStartTimeRef.current) / 60000;
        const words = finalTranscript.split(/\s+/).filter(w => w.length > 0).length;
        if (durationMins > 0) wpm = Math.round(words / durationMins);
      }
  ...
  }, [cleanupAudioSession, setAuraSpeaking]);
